...
public class JasperHtmlServlet extends HttpServlet {
	...
	private transient Logger log = LoggerFactory.getLogger(JasperHtmlServlet.class);
...
//			JasperExportManager.exportReportToPdfStream(print, resp.getOutputStream());
			File tmpFile = File.createTempFile("jaspertemp", ".html");
			JasperExportManager.exportReportToHtmlFile(print, tmpFile.getPath());
			IOUtils.copy(new FileReader(tmpFile), resp.getOutputStream());
